...
func main() {
	...
	fmt.Fprintf(out, "\tlicenses = []license{\n")
	for _, file := range files {
		fmt.Fprintf(out, "\t{name: %q, ", filepath.Base(file))
		fmt.Fprintf(out, "doc: normalize([]byte(%s))},\n", varName(file))
	}
	fmt.Fprintf(out, "\t}\n")
	fmt.Fprintf(out, "\tfor i := range licenses {\n")
	fmt.Fprintf(out, "\t\tl := &licenses[i]\n")
	fmt.Fprintf(out, "\t\tl.typ = licenseType(l.name)\n")
	fmt.Fprintf(out, "\t\tl.startIndexes = startIndexes(l.doc.words)\n")
	fmt.Fprintf(out, "\t}\n")
	fmt.Fprintf(out, "}\n")
	fmt.Fprintf(out, "\n")
